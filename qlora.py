...
import evaluate
from load_rz_format import load_from_path
...
class TrainingArguments(transformers.Seq2SeqTrainingArguments):
    ...
    checkpoint_dir: str = field(
        default=None, metadata={"help": "The checkpoint directory to load from."}
    )
    optim: str = field(
...
class GenerationArguments:
    ...
    yes_no: Optional[bool] = field(default=False)
...
def train():
    ...
    args_path = os.path.join(args.output_dir, "args.pkl")
    ...
    if args.do_predict:
        if not args.yes_no:
            ...
            data_loader = trainer.get_eval_dataloader(data_module["predict_dataset"])
            ...
            for batch in tqdm(data_loader, total=len(data_loader)):
                ...
                for i, logit in enumerate(logits):
                    label_non_zero_id = (batch["labels"][i] != -100).nonzero()[0][0]
                    logit_yes_no = logit[label_non_zero_id - 1][yes_no_idx]
                    preds.append(torch.argmax(logit_yes_no).item())
                labels = labels[labels != IGNORE_INDEX].view(-1, 2)[:, 0]
                refs += [yes_no_idx.index(label) for label in labels.tolist()]
